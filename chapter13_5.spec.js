...
test("Verify the functionality of BMI Calculator", async({page}) =>{
    ...
    await test.step("priority of one", async()=>{
        await page.locator('#cage').fill('30');
    ...
    await test.step("priority of three", async()=>{
        await page.locator('#cage').fill('69');
        await page.locator('#cheightmeter').fill('170');
        await page.locator('#ckg').fill('70');
        await page.locator('input[type*="submit"]').click();
        expect(await page.locator('.bigtext > b')).toHaveText('BMI = 24.2 kg/m2')
    });
